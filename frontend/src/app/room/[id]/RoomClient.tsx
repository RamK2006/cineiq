    ...
    ws.current.onmessage = (event) => {
      try {
        ...
        if (message.type === 'play') {
          ...
          if (!userIdRef.current) userIdRef.current = message.user;
          ...
          // Backward compatibility if backend sends sync event instead
    ...
    <main className="room-container" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#000' }}>
      ...
      <div className="glass-panel room-sidebar" style={{ position: 'absolute', top: '80px', right: '20px', bottom: '20px', width: '280px', display: 'flex', flexDirection: 'column', border: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <div style={{ flex: 1, padding: '16px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {messages.map((m, i) => (
            <div key={i} style={{ background: m.user === userName ? 'rgba(229,9,20,0.1)' : 'rgba(255,255,255,0.05)', padding: '10px 14px', borderRadius: '12px', alignSelf: m.user === userName ? 'flex-end' : 'flex-start', maxWidth: '85%' }}>
