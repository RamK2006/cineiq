        ...
        for peer_id, client_ws in list(ROOM_SIGNALING_REGISTRY[room_id].items()):
            if peer_id != sender_id:
                try:
                    ...
                    logger.error(
                        f"Signaling dispatch failed from {sender_id} to {peer_id}: {str(e)}"
                    )


async def broadcast_to_room(
    room_id: str, message: dict, exclude_user_id: Optional[str] = None
):
        ...
        for uid, client in list(ROOM_PRESENCE_REGISTRY[room_id].items()):
            if exclude_user_id is None or uid != exclude_user_id:
                try:
                    ...
                    logger.error(
                        f"Failed broadcasting payload in room {room_id}: {str(e)}"
                    )
...
async def room_websocket_signaling_endpoint(
    ...
    ROOM_PRESENCE_REGISTRY[room_id][user_id] = {
        ...
        "username": username,
        "avatar": avatar
        or f"https://ui-avatars.com/api/?name={username}&background=random",
    }
    ...
    # Fetch recent 50 chat messages from Redis or memory for newly joined participants
    recent_history = []
    redis = get_redis()
    if redis:
        try:
            ...
            if raw_history:
                recent_history = [json.loads(m) for m in raw_history]
        except Exception as e:
            logger.warning("redis_history_fetch_failed", error=str(e))
    ...
    await broadcast_to_room(
        room_id,
        {
            ...
            "data": {
                "userId": user_id,
                "username": username,
                "avatar": ROOM_PRESENCE_REGISTRY[room_id][user_id]["avatar"],
            },
        },
    ...
    await broadcast_to_room_peers(
        room_id, user_id, {"type": "peer-joined", "peerId": user_id}
    )
    ...
    try:
        while True:
            ...
            if event_type == "CHAT_MESSAGE":
                ...
                msg_payload = {
                    ...
                    or time.strftime("%H:%M", time.gmtime()),
                }

                # Store in Redis with LTRIM to cap at 50 messages
                if redis:
                    try:
                ...
                await broadcast_to_room(
                    room_id, {"type": "CHAT_MESSAGE", "data": msg_payload}
                )
        ...
        if (
            room_id in ROOM_SIGNALING_REGISTRY
            and user_id in ROOM_SIGNALING_REGISTRY[room_id]
        ):
        ...
        if (
            room_id in ROOM_PRESENCE_REGISTRY
            and user_id in ROOM_PRESENCE_REGISTRY[room_id]
        ):
            del ROOM_PRESENCE_REGISTRY[room_id][user_id]
            if not ROOM_PRESENCE_REGISTRY[room_id]:
                del ROOM_PRESENCE_REGISTRY[room_id]

        # Clean up memory history if room is empty (Redis handles its own expiration)
        if (
            not redis
            and room_id in ROOM_CHAT_HISTORY
            and not ROOM_CHAT_HISTORY[room_id]
        ):
            del ROOM_CHAT_HISTORY[room_id]

        # Broadcast USER_LEFT to maintain client state consistency
        await broadcast_to_room(
            room_id, {"type": "USER_LEFT", "data": {"userId": user_id}}
        )
        await broadcast_to_room_peers(
    ...
    if redis:
        try:
            redis.set(
                ...
                json.dumps(
                    {"action": "pause", "progress": 0, "timestamp": time.time()}
                ),
            )
        ...
        try:
            ...
            if state_data:
                await websocket.send_json(
                    {"type": "sync", "payload": json.loads(state_data)}
                )
        except Exception as e:
                    ...
                    if msg_type == "TRANSFER_HOST":
                        ...
                        if (
                            ...
                            if target_ws:
                                ...
                                await manager.broadcast(
                                    room_id,
                                    {"type": "user_left", "user": target_id},
                                    sender=websocket,
                                )
                        else:
                            await manager.send_personal_message(
                                {"error": "Invalid user to kick"}, websocket
                            )
